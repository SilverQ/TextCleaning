...
def extract_clean_text(input_df, pos_level=3):
    ...
    input_df['title1'] = input_df['title_pos'].apply(lambda x: [word[0] for word in x if word[1] not in stop_pos_list])
    input_df['title2'] = input_df['title1'].apply(lambda x: [w for w in x if len(w) > 1])
    print(proc_time(starting_time), ' 5. Filter with pos info')

    title_clean3 = [' '.join(sen) for sen in input_df['title2'].tolist()]
    return title_clean3
...
def sna_bipartite(input_df, node_col, timestamp_col, edge_col):
    ...
    df_node = df_node.groupby(['Timestamp', node_col]).count()
    df_node = df_node.reset_index()
    print(df_node.head(5))
    df_node['Id'] = df_node[node_col]
    df_node['Type'] = 'Undirected'
    df_node.columns = ['Timestamp', 'Id', 'Cnt', 'Label', 'Type']
    print(df_node.head(5))
